...
Reference Dose (RfD) is based on the human equivalent dose of where there's either no observable adverse health effects (NOAEL) level or low observable adverse health effects level (LOAEL). It is buffered by an uncertainty factor (UF) which adjusts for the interspecies variabilty (e.g. from mice to humans). The RfD is then modified by body weight and the ingestion (BW/I) ratio to calculate the drinking water equivalent level (DWEL). A reduced BW/I ratio lowers the range of possible lifetime health advisory (Lifetime HA) given a range of relative source contribution (RSC) values. It is important to note that EPA uses 0.2 (20%) as a default RSC. RSC of 0.2 is saying that 20% of your contribution to this contaminant comes from drinking water. *Something to think about: If DWELL is 1 and RSC is 1 (100%), then your MCLG would be 1. If your DWELL was 1 and the RSC was 0.2 (20%), your MCLG would be 0.2ng/L*. This means, the lower the relative source contribution, the lower it drives the MCLG! A lot of people argue that this is problematic (see PFAS for example, using RSC ranging from 0.2 to 1 can drive MCLG anywhere from 70ng/L to >600ng/L, big range!). In short, Lifetime HA (or MCLG), is significantly influenced by RfD, RSC and BW/I values. 

There can be multiple RfD sources whose standard are set by different agencies. For example for PFOS/PFOA, ASTDR (CDC), CA (California), NJ (New Jersey), and IRIS (USEPA) are different RfD sources used by different agencies which uses different RfD values. The difference in RfD is most likely due to the different UF used in calculation.
